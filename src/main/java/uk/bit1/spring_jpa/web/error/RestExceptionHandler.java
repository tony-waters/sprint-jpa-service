...
public class RestExceptionHandler {
    ...
    public ApiError notFound(NotFoundException ex, HttpServletRequest req) {
        return new ApiError(Instant.now(), 404, "NOT_FOUND", ex.getMessage(), req.getRequestURI(), Map.of());
    }

    @ExceptionHandler(IllegalArgumentException.class)
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ApiError badRequest(IllegalArgumentException ex, HttpServletRequest req) {
        return new ApiError(Instant.now(), 400, "BAD_REQUEST", ex.getMessage(), req.getRequestURI(), Map.of());
    }

    @ExceptionHandler(IllegalStateException.class)
    @ResponseStatus(HttpStatus.CONFLICT)
    public ApiError conflict(IllegalStateException ex, HttpServletRequest req) {
        return new ApiError(Instant.now(), 409, "CONFLICT", ex.getMessage(), req.getRequestURI(), Map.of());
    }
    ...
    public ApiError validation(MethodArgumentNotValidException ex, HttpServletRequest req) {
        ...
        return new ApiError(
                Instant.now(), 400, "VALIDATION_ERROR", "Request validation failed", req.getRequestURI(), fieldErrors
        );
    }
}